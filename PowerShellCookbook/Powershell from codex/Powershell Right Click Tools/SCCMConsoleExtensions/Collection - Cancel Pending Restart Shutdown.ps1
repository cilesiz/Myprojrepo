...
$FormName = "Cancel Pending Restart/Shutdown - $ColName"
$strActionLbl = "Cancel Pending Restart/Shutdown"
...
$ReRunBtn_OnClick= 
{
	...
	$ArgList += @("`"$Directory\Collection - Cancel Pending Restart Shutdown.ps1`"")
	$ArgList += @("`"$ColID`"")
	$ArgList += @("`"$ColName`"")
...
$OnLoadForm_StateCorrection=
{
	...
	Get-WmiObject -Query $strQuery -Namespace $Namespace -ComputerName $Server | ForEach-Object {
		...
		if ($script:CancelAction -eq $false){
			$CompName = $_.Name
			$JobName = "CancelShutDownRestart_" + $CompName
			$CurrTime = Get-Date
			$JobTimer.add("$CompName",$CurrTime)
			$CurrentTime = $CurrTime.ToLongTimeString()
			$LogText = "$CurrentTime - Cancelling pending Shutdown/Restart on $CompName`n"
			...
			Start-Job -Name $JobName -ArgumentList $CompName -ScriptBlock {
				...
				If (test-connection -computername $CompName -count 1 -quiet){
					...
					if ($ShutdownExitCode -eq 1116) {$strOutput = "$CompName" + "||Successful||No pending shutdown"}
					elseif ($ShutdownExitCode -eq 0) {$strOutput = "$CompName" + "||Successful||Cancelled pending shutdown"}
			...
			[System.Windows.Forms.Application]::DoEvents()
			Receive-Job -Name "CancelShutDownRestart_*" | ForEach-Object {
				[System.Windows.Forms.Application]::DoEvents()
				$count++
				$Output = $_
				foreach ($strOutput in $Output) {
					if ($strOutput.contains("||")){
						...
						elseif ($strOutput.contains("||Error")) {
							...
							$OutputArray = $strOutput.Split("||")
							$SuccessView.Rows.Add($OutputArray[0],$OutputArray[4])
							$LogBox.Text = $LogBox.Text + "$CurrentTime - Cancelled Shutdown/Restart on " + $OutputArray[0] + "`n"
							$Numsuccess++
			...
			do {
				...
				$RunningJobs = 0
				$IgnoredJobs = 0
				get-job | where-object {$_.Name -like "CancelShutDownRestart_*" -and $_.State -eq "Running"} | ForEach-Object {
					[System.Windows.Forms.Application]::DoEvents()
					$JobID = $_.ID
					if ($SkippedJobs -inotcontains "$JobID") {
						...
						$CurrentTime = $CurrTime.ToLongTimeString()
						$JobCompName = $_.Name
						$JobCompName = $JobCompName.replace("CancelShutDownRestart_","")
