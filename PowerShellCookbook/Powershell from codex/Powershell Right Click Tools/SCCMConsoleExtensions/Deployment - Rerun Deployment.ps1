...
$ReRunBtn_OnClick= 
{
	...
	$ArgList += @("`"$Directory\Deployment - Rerun Deployment.ps1`"")
...
$OnLoadForm_StateCorrection=
{
	...
	foreach($CompName in $CompList) {
		...
		if ($script:CancelAction -eq $false){
			...
			$AdvID = $DeploymentID
			$CurrTime = Get-Date
			$JobName = "RerunDeploy_" + $CompName
			$JobTimer.add("$CompName",$CurrTime)
			$CurrentTime = $CurrTime.ToLongTimeString()
			$LogText = "$CurrentTime - Starting deployment rerun on $CompName"
			$LogText = $LogText.replace("`n","")
			$LogBox.Text = $LogBox.Text + $LogText + "`n"
			$LogScrollTo = $LogBox.Text.Length - 250
			$LogBox.Select($LogScrollTo,0)
			$LogBox.ScrollToCaret()
			Start-Job -Name $JobName -ArgumentList $CompName,$AdvID -ScriptBlock {
				$CompName = $null
				$CompName = $args[0]
				$AdvID = $args[1]
			...
			Receive-Job -Name "RerunDeploy_*" | ForEach-Object {
				[System.Windows.Forms.Application]::DoEvents()
				$count++
				$Output = $_
			...
			do {
				...
				$RunningJobs = 0
				$IgnoredJobs = 0
				get-job | where-object {$_.Name -like "RerunDeploy_*" -and $_.State -eq "Running"} | ForEach-Object {
					[System.Windows.Forms.Application]::DoEvents()
					$JobID = $_.ID
					if ($SkippedJobs -inotcontains "$JobID") {
						...
						$CurrentTime = $CurrTime.ToLongTimeString()
						$JobCompName = $_.Name
						$JobCompName = $JobCompName.replace("RerunDeploy_","")
...
$Answer = $Popup.Popup("Are you sure you want to rerun the deployment $DeploymentName",0,"Are you sure?",1)
