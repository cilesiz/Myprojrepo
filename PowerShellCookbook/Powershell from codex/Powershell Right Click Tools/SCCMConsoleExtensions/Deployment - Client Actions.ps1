...
if ($Tool -eq "Application Deployment Evaluation Cycle") {$strAction = "{00000000-0000-0000-0000-000000000121}"}
elseif ($Tool -eq "Discovery Data Collection Cycle") {$strAction = "{00000000-0000-0000-0000-000000000003}"}
elseif ($Tool -eq "Hardware Inventory Cycle") {$strAction = "{00000000-0000-0000-0000-000000000001}"}
elseif ($Tool -eq "Machine Policy Retrieval and Evaluation Cycle") {$strAction = "{00000000-0000-0000-0000-000000000021}"}
elseif ($Tool -eq "Software Inventory Cycle") {$strAction = "{00000000-0000-0000-0000-000000000002}"}
elseif ($Tool -eq "Software Metering Usage Report Cycle") {$strAction = "{00000000-0000-0000-0000-000000000031}"}
elseif ($Tool -eq "Software Updates Deployment Evaluation Cycle") {$strAction = "{00000000-0000-0000-0000-000000000108}"}
elseif ($Tool -eq "Software Updates Scan Cycle") {$strAction = "{00000000-0000-0000-0000-000000000113}"}
elseif ($Tool -eq "Windows Installer Source List Update Cycle") {$strAction = "{00000000-0000-0000-0000-000000000032}"}

$FormName = "$Tool - $DeploymentName"
$strActionLbl = "$Tool"
...
$DeploymentName - Collection name for the DeployNameLbl
$StrActionLbl - Action name for the RunningOnLbl
...
$OnLoadForm_StateCorrection=
{
	...
	foreach($CompName in $CompList) {
		...
		if ($script:CancelAction -eq $false){
			<#There is a bug with Start-Job that causes all jobs to complete before it removes any from memory. 
			...
			$CurrTime = Get-Date
			$JobName = "DeployAction_" + $CompName
			$JobTimer.add("$CompName",$CurrTime)
			$CurrentTime = $CurrTime.ToLongTimeString()
			$LogText = "$CurrentTime - Starting $Tool on $CompName"
			$LogText = $LogText.replace("`n","")
			$LogBox.Text = $LogBox.Text + $LogText + "`n"
			$LogScrollTo = $LogBox.Text.Length - 250
			$LogBox.Select($LogScrollTo,0)
			$LogBox.ScrollToCaret()
			Start-Job -Name $JobName -ArgumentList $CompName,$strAction -ScriptBlock {
				$CompName = $null
				$CompName = $args[0]
				$strAction = $args[1]
			...
			Receive-Job -Name "DeployAction_*" | ForEach-Object {
				[System.Windows.Forms.Application]::DoEvents()
				$count++
				$strOutput = $_
				...
				elseif ($strOutput.contains("||Error")){
					...
					$LogBox.Text = $LogBox.Text + "$CurrentTime - Successfully triggered $Tool on " + $OutputArray[0] + "`n"
					$Numsuccess++
			...
			do {
				...
				$RunningJobs = 0
				$IgnoredJobs = 0
				get-job | where-object {$_.Name -like "DeployAction_*" -and $_.State -eq "Running"} | ForEach-Object {
					[System.Windows.Forms.Application]::DoEvents()
					$JobID = $_.ID
					if ($SkippedJobs -inotcontains "$JobID") {
						...
						$CurrentTime = $CurrTime.ToLongTimeString()
						$JobCompName = $_.Name
						$JobCompName = $JobCompName.replace("DeployAction_","")
...
$ResultsForm.Controls.Add($DeployNameLbl)
