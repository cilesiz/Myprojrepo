...
$FormName = "Change Cache Size - $DeploymentName"
...
$DeploymentName - Collection name for the ColNameLbl
$RunningOnLbl - Action name for the RunningOnLbl
...
$ReRunBtn_OnClick= 
{
	...
	$ArgList += @("`"$Directory\Deployment - Change Cache.ps1`"")
...
$OnLoadForm_StateCorrection=
{
	...
	$System_Windows_Forms_DataGridViewTextBoxColumn_2.HeaderText = "New Cache Size (MB)"
	...
	$System_Windows_Forms_DataGridViewTextBoxColumn_3.HeaderText = "Previous Cache Size (MB)"
	...
	$System_Windows_Forms_DataGridViewTextBoxColumn_4.HeaderText = "Size of Objects in Cache (MB)"
	...
	do {
		$NewCacheSize = [Microsoft.VisualBasic.Interaction]::InputBox("Please enter the new cache size in MB","Change cache size","");
		$refnum = 0
		if ($NewCacheSize -ne "" -and [System.Int32]::TryParse($NewCacheSize, [ref]$refnum)){$x=1}
		elseif ($NewCacheSize.length -eq 0){$ResultsForm.close()}
	} while ($x -ne 1)
	$PopupAnswer = $Popup.popup("Do you want to change the cache to $NewCacheSize MB?",0,"Are you sure?",1)
	if ($PopupAnswer -ne 1) {$ResultsForm.close()}
	...
	foreach($CompName in $CompList) {
		...
		if ($script:CancelAction -eq $false){
			$JobName = "CacheSize_" + $CompName
			$CurrTime = Get-Date
			$JobTimer.add("$CompName",$CurrTime)
			$CurrentTime = $CurrTime.ToLongTimeString()
			$LogText = "$CurrentTime - Changing cache of $CompName`n"
			$LogBox.Text = $LogBox.Text + $LogText
			$LogScrollTo = $LogBox.Text.Length - 250
			$LogBox.Select($LogScrollTo,0)
			$LogBox.ScrollToCaret()
			Start-Job -Name $JobName -ArgumentList $CompName,$NewCacheSize -ScriptBlock {
				$CompName = $args[0]
				$NewCacheSize = $args[1]
				If (test-connection -computername $CompName -count 1 -quiet){
					...
					else{$strOutput = $CompName + "||" + $NewCacheSize + "||" + $OldCache + "||" + $CacheUsed}
				}
				else {$strOutput = "$Compname ||Off"}
				Write-Output $strOutput
			} | Out-Null
			[System.Windows.Forms.Application]::DoEvents()
			Receive-Job -Name "CacheSize_*" | ForEach-Object {
				[System.Windows.Forms.Application]::DoEvents()
				$count++
				$strOutput = $_
				...
				elseif ($strOutput.contains("||Error")) {
					...
					$LogBox.Text = $LogBox.Text + "$CurrentTime - Successfully changed cache of " + $OutputArray[0] + "`n"
					$Numsuccess++
			...
			do {
				...
				$RunningJobs = 0
				$IgnoredJobs = 0
				get-job | where-object {$_.Name -like "CacheSize_*" -and $_.State -eq "Running"} | ForEach-Object {
					[System.Windows.Forms.Application]::DoEvents()
					$JobID = $_.ID
					if ($SkippedJobs -inotcontains "$JobID") {
						...
						$CurrentTime = $CurrTime.ToLongTimeString()
						$JobCompName = $_.Name
						$JobCompName = $JobCompName.replace("CacheSize_","")
	...
	do {
		...
		if ($RunningJobs -gt 0) {
			[System.Windows.Forms.Application]::DoEvents()
			Start-Sleep 1
			$LogBox.Text = $LogBox.Text + "$CurrentTime - Waiting on $RunningJobs jobs to finish `n"
			$LogScrollTo = $LogBox.Text.Length - 250
			$LogBox.Select($LogScrollTo,0)
			$LogBox.ScrollToCaret()
		}
...
$RunningOnLbl.Text = "$RunningOnLbl"
...
$DeploymentNameLbl.Name = "ColNameLbl"
$System_Drawing_Size = New-Object System.Drawing.Size
$System_Drawing_Size.Height = 23
$System_Drawing_Size.Width = 505
