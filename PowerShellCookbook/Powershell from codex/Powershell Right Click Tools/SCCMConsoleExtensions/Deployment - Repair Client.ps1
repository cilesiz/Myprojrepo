...
$FormName = "Repair Client - $DeploymentName"
$strActionLbl = "Repair Client"
...
$ReRunBtn_OnClick= 
{
	...
	$ArgList += @("`"$Directory\Deployment - Repair Client.ps1`"")
...
$OnLoadForm_StateCorrection=
{
	...
	foreach($CompName in $CompList) {
		...
		if ($script:CancelAction -eq $false){
			...
			$CurrTime = Get-Date
			$JobName = "RepairCol_" + $CompName
			$JobTimer.add("$CompName",$CurrTime)
			$CurrentTime = $CurrTime.ToLongTimeString()
			$LogText = "$CurrentTime - Starting client repair on $CompName"
			...
			Receive-Job -Name "RepairCol_*" | ForEach-Object {
				[System.Windows.Forms.Application]::DoEvents()
				$count++
				$strOutput = $_
				...
				elseif ($strOutput.contains("||Error")){
					...
					$LogBox.Text = $LogBox.Text + "$CurrentTime - Successfully repaired client on " + $OutputArray[0] + "`n"
					$Numsuccess++
			...
			do {
				...
				$RunningJobs = 0
				$IgnoredJobs = 0
				get-job | where-object {$_.Name -like "RepairCol_*" -and $_.State -eq "Running"} | ForEach-Object {
					[System.Windows.Forms.Application]::DoEvents()
					$JobID = $_.ID
					if ($SkippedJobs -inotcontains "$JobID") {
						...
						$CurrentTime = $CurrTime.ToLongTimeString()
						$JobCompName = $_.Name
						$JobCompName = $JobCompName.replace("RepairCol_","")
...
$Answer = $Popup.Popup("Are you sure you want to repair the client on deployment $DeploymentName",0,"Successful",1)
