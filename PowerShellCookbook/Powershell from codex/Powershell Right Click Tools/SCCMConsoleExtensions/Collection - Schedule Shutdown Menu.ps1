...
$Start_Btn_Action= 
{
	...
	elseif ($FreqText.contains("On Day") -and $Modifier -eq $null) {$Popup.Popup("You need to select a modifier when choosing Monthly (On Day)",0,"Error",16)}
		...
		$ArgList += @("`"$Directory\Collection - Schedule Shutdown.ps1`"")
		$ArgList += @("`"$ColID`"")
		$ArgList += @("`"$ColName`"")
		$ArgList += @("`"$Server`"")
		$ArgList += @("`"$Namespace`"")
...
{#Correct the initial state of the form to prevent the .Net maximized form issue
	...
	$CompColName.Text = "Schedule Restart/Shutdown - $ColName"
...
$form1.Name = "form1"
$form1.Text = "Schedule Restart/Shutdown - Collection"
