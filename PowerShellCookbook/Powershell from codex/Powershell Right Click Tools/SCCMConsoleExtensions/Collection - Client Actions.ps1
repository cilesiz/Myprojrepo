...
$FormName = "$strActionName - $ColName"
$strActionLbl = "$strActionName"
...
$ReRunBtn_OnClick= 
{
	...
	$ArgList += @("`"$Directory\ClientActionsCol.ps1`"")
	$ArgList += @("`"$ColID`"")
	$ArgList += @("`"$ColName`"")
...
$OnLoadForm_StateCorrection=
{
	...
	Get-WmiObject -Query $strQuery -Namespace $Namespace -ComputerName $Server | ForEach-Object {
		...
		if ($script:CancelAction -eq $false){
			...
			$CurrTime = Get-Date
			$JobName = "ColAction_" + $CompName
			$JobTimer.add("$CompName",$CurrTime)
			$CurrentTime = $CurrTime.ToLongTimeString()
			$LogText = "$CurrentTime - Starting $strActionName on $CompName"
			...
			Receive-Job -Name "ColAction_*" | ForEach-Object {
				[System.Windows.Forms.Application]::DoEvents()
				$count++
				$strOutput = $_
				...
				elseif ($strOutput.contains("||Error")){
					...
					$LogBox.Text = $LogBox.Text + "$CurrentTime - Successfully triggered $strActionName on " + $OutputArray[0] + "`n"
					$Numsuccess++
			...
			do {
				...
				$RunningJobs = 0
				$IgnoredJobs = 0
				get-job | where-object {$_.Name -like "ColAction_*" -and $_.State -eq "Running"} | ForEach-Object {
					[System.Windows.Forms.Application]::DoEvents()
					$JobID = $_.ID
					if ($SkippedJobs -inotcontains "$JobID") {
						...
						$CurrentTime = $CurrTime.ToLongTimeString()
						$JobCompName = $_.Name
						$JobCompName = $JobCompName.replace("ColAction_","")
