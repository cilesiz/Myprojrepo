...
$FormName = "Clear Cache - $DeploymentName"
...
$ReRunBtn_OnClick= 
{
	...
	$ArgList += @("`"$Directory\Deployment - Clear Cache.ps1`"")
...
$OnLoadForm_StateCorrection=
{
	...
	foreach($CompName in $CompList) {
		...
		if ($script:CancelAction -eq $false){
			...
			$CurrTime = Get-Date
			$JobName = "ClearCache_" + $CompName
			$JobTimer.add("$CompName",$CurrTime)
			$CurrentTime = $CurrTime.ToLongTimeString()
			$LogText = "$CurrentTime - Clearing cache of $CompName"
			...
			Receive-Job -Name "ClearCache_*" | ForEach-Object {
				[System.Windows.Forms.Application]::DoEvents()
				$count++
				$strOutput = $_
				...
				elseif ($strOutput.contains("||Error")){
					...
					$LogBox.Text = $LogBox.Text + "$CurrentTime - Successfully cleared cache on " + $OutputArray[0] + "`n"
					$Numsuccess++
			...
			do {
				...
				$RunningJobs = 0
				$IgnoredJobs = 0
				get-job | where-object {$_.Name -like "ClearCache_*" -and $_.State -eq "Running"} | ForEach-Object {
					[System.Windows.Forms.Application]::DoEvents()
					$JobID = $_.ID
					if ($SkippedJobs -inotcontains "$JobID") {
						...
						$CurrentTime = $CurrTime.ToLongTimeString()
						$JobCompName = $_.Name
						$JobCompName = $JobCompName.replace("ClearCache_","")
...
$Answer = $Popup.Popup("Are you sure you want to clear the cache on $DeploymentName",0,"Are you sure?",1)
