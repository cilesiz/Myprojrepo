...
$FormName = "Uninstall Client - $DeploymentName"
...
$ReRunBtn_OnClick= 
{
	...
	$ArgList += @("`"$Directory\Deployment - Uninstall Client.ps1`"")
...
$OnLoadForm_StateCorrection=
{
	...
	foreach($CompName in $CompList) {
		...
		if ($script:CancelAction -eq $false){
			...
			$CurrTime = Get-Date
			$JobName = "UninstallCol_" + $CompName
			$JobTimer.add("$CompName",$CurrTime)
			$CurrentTime = $CurrTime.ToLongTimeString()
			$LogText = "$CurrentTime - Starting client uninstall on $CompName"
			...
			Start-Job -Name $JobName -ArgumentList $CompName -ScriptBlock {
				...
				If (test-connection -computername $CompName -count 1 -quiet){
					...
					if ($Error[0]) {$strOutput = "$CompName" + "||Error|| " + $Error}
						...
						else {$strOutput = "$CompName" + "||Error|| $ProcError " + $Error}
					}
				}
				else {$strOutput = "$CompName" + "||Off"}
				Write-Output $strOutput
			} | Out-Null
			Receive-Job -Name "UninstallCol_*" | ForEach-Object {
				[System.Windows.Forms.Application]::DoEvents()
				$count++
				$strOutput = $_
				...
				elseif ($strOutput.contains("||Error")){
					...
					$LogBox.Text = $LogBox.Text + "$CurrentTime - Successfully uninstalled client on " + $OutputArray[0] + "`n"
					$Numsuccess++
			...
			do {
				...
				$RunningJobs = 0
				$IgnoredJobs = 0
				get-job | where-object {$_.Name -like "UninstallCol_*" -and $_.State -eq "Running"} | ForEach-Object {
					[System.Windows.Forms.Application]::DoEvents()
					$JobID = $_.ID
					if ($SkippedJobs -inotcontains "$JobID") {
						...
						$CurrentTime = $CurrTime.ToLongTimeString()
						$JobCompName = $_.Name
						$JobCompName = $JobCompName.replace("UninstallCol_","")
...
$Answer = $Popup.Popup("Are you sure you want to uninstall the client on Deployment $DeploymentName",0,"Are you sure?",1)
