...
$thisScript.WinOS      = (Get-WmiObject Win32_OperatingSystem)
$thisScript.WinVer     = [int]$thisScript.WinOS.Version.Split(".")[0]
$thisScript.HostVer    = [int](Get-Host).Version.Major
...
Function CheckVars {
	...
	# Check we're on Powershell 3.x
	If(!($thisScript.Hostver -ge 2)) {
		$errCheckVars += ("You must be on PowerShell 2.x (or better) to run this script. You're on {0}" -f $thisScript.Hostver)
	}
	...
	If($BkWorkDrive -ieq "") { 
		...
		If(($BkWorkDrive -is [array])) 	{ 
			...
			If(($BkWorkDrive.Length -gt 1)) { 
				$errCheckVars += "Value for --workdrive must be a valid drive letter." 
			} Else {
				If(!($BkWorkDrive -match "^[C-Zc-z]")) { 
					$errCheckVars += "Value for --workdrive must be a valid drive letter." 
				} Else {
			...
			If(!(Test-Path $BkSelection -pathType Leaf)) { 
				...
				If((CheckFSAttribute $BkSelection "Directory")) {
					...
					If((Get-ChildItem $BkSelection).Length -eq 0) { 
						$errCheckVars += "File provided for --selection is zero length" 
					} Else {
						...
						Remove-Variable -name BkSelectionContents -scope Script -ErrorAction SilentlyContinue
						Get-Content $BkSelection | Where-Object { ($_ -match "^prefix=*|^includesource=*|^matchincludefiles=*|^matchexcludefiles=*|^matchcleanupfiles=*|^matchexcludepath=*|^maxfileage=[0-9]|^minfileage=[0-9]|^maxfilesize=[0-9]|^minfilesize=[0-9]|^matchstoprecurse=*|^maxdepth=[0-9]|^rotate=[0-9]|^nofollowjunctions$|^useswitches=*") } | Set-Variable -name BkSelectionContents -scope Script
						...
						If(!(IsVarDefined "BkSelectionContents")) {
							...
							If( ! (($BkSelectionContents | Where-Object {$_ -match "^includesource=*"}).Length -gt 0) ) {
								...
								# Look wether selection contents holds specific 7zip switches to use.
								...
								# Look wether selection contents holds specific maxdepth value to use.
								...
								# Look wether selection contents holds specific rotate value to use.
								...
								# Look wether selection contents holds specific prefix value to use.
	...
	If(!(IsVarDefined "BkLogFile")) { 
		Set-Variable -name BkLogFile -value (Join-Path $Env:Temp ($thisScript.Name.Substring(0, ($thisScript.Name.LastIndexOf("."))) + (Get-Date -format "yyyyMMdd-HHmmss") + ".log")) -scope Script
	} 
	If($BkLogFile -ieq "") { 
	...
	If((IsVarDefined "BkNotifyLog")) {
		...
		If(!(IsVarDefined "BkNotifyExtra")) { 
			...
			Switch ($BkNotifyExtra) {
				"none"   { Set-Variable -name BkArchiveType -value "none"  -scope Script }
				"inline" { Set-Variable -name BkArchiveType -value "inline" -scope Script }
				"attach" { Set-Variable -name BkArchiveType -value "attach" -scope Script }
	...
	If(!(IsVarDefined "Bk7ZipBin")) { 
		Set-Variable -name Bk7ZipBin -value (Join-Path $Env:ProgramFiles "\7-Zip\7z.exe") -scope Script
	} 
	If($Bk7ZipBin -ieq "") { 
...
# -----------------------------------------------------------------------------
# Description	: Checks wether or not the user hit CTRL + C to request
#                 script cancel
# Parameters    : 
# Returns       : $True / $False
# Credits       : 
...
Function CleanUp {
	...
	If ((IsVarDefined "cmdLineBatch")) {if ((Test-Path ($cmdLineBatch))) { Remove-Item -literalpath $cmdLineBatch -ErrorAction SilentlyContinue | Out-Null }}
	If ((IsVarDefined "BkLogFile")) {if ((Test-Path ($BkLogFile))) { Remove-Item -literalpath $BkLogFile -ErrorAction SilentlyContinue | Out-Null }}
	If ((IsVarDefined "BkLockFile")) {if ((Test-Path ($BkLockFile))) { Remove-Item -literalpath $BkLockFile -ErrorAction SilentlyContinue | Out-Null }}
...
Function CheckFSAttribute {
	...
	$item = (Get-Item -literalPath $itemFullName -force -ErrorAction SilentlyContinue)
	If(($item)){
...
Function DeleteRootDir {
	...
	if ((Test-Path $rootPath -PathType Container)) {
		...
		Remove-Item (Join-Path $rootPath "Selection-Info.txt") -ErrorAction SilentlyContinue| Out-Null
		Remove-Item (Join-Path $rootPath "Selection-Excpt.txt") -ErrorAction SilentlyContinue| Out-Null
		Remove-Item (Join-Path $rootPath "Catalog-Include.txt") -ErrorAction SilentlyContinue| Out-Null
		Remove-Item (Join-Path $rootPath "Catalog-Exclude.txt") -ErrorAction SilentlyContinue| Out-Null
		Remove-Item (Join-Path $rootPath "Catalog-Stats.txt") -ErrorAction SilentlyContinue| Out-Null
		Remove-Item (Join-Path $rootPath "Compress-Detail.txt") -ErrorAction SilentlyContinue| Out-Null
		Remove-Item (Join-Path $rootPath "__README__PLEASE__README__.txt") -ErrorAction SilentlyContinue| Out-Null
		Remove-Item $rootPath -ErrorAction SilentlyContinue | Out-Null
		If(!$?) { $False } Else { $True }
	} Else { $False }
...
Function IsVarDefined { 
	...
	Get-Variable -name $varName -scope Script -ErrorAction SilentlyContinue | Out-Null
	If(!$?) { $False } Else { $True	}
}
...
Function PostArchiving {
	...
	If(!($bkType -ieq "move")) {
		...
		If(($BkClearBit -eq $True)) {
			...
			Trace " ----------------------------------------" $BkLogFile
			$i = 0
			Get-Content $BkCompressDetail -encoding UTF8 | Where-Object {$_ -match "^Compressing\ \ "} | Sort | ForEach-Object {
				...
				$fileName = $_.Substring(13)
				Write-Progress -Activity "Clearing Archive Bit" -PercentComplete $percentCompleted -Status "Please wait ..." -CurrentOperation ($fileName)
				If(!(Clear-FileAttribute (Join-Path $BkRootDir $fileName) Archive)) { $gP.WarningsCount += 1 }
			}
		...
		Get-Content $BkCompressDetail -encoding UTF8 | Where-Object {$_ -match "^Compressing\ \ "} | Sort | ForEach-Object {
			...
			$i++ ; $percentCompleted = ( $i / $gP.FilesSelected * 100 ) ; If(($percentCompleted -gt 100)) { $percentCompleted = 100 }
			$fileName = $_.Substring(13)
			Write-Progress -Activity "Deleting files" -PercentComplete $percentCompleted -Status ($fileName) -CurrentOperation "Please Wait ..."
			Remove-Item (Join-Path $BkRootDir $fileName) -ErrorAction SilentlyContinue | Out-Null
			if (!($?)) { Trace (" WARNING Failed to remove : $fileName") $BkLogFile; $gP.WarningsCount += 1 }
		}
...
Function ProcessFolder ([string]$folderPath, [int]$depth) {
	...
	# Status
	Write-Progress -Activity "Folder $thisRelativePath" -Status "File Select : " -CurrentOperation ("Selected {0,0:n0} out of {1,0:n0} files in {2,0:n0} folders. {3,0:n2} MBytes to backup" -f  $gP.FilesSelected, $gP.FilesProcessed, $gP.FoldersDone, ($gP.BytesSelected/1mb))
	...
	Get-ChildItem $folderPath -force -ErrorAction SilentlyContinue -ErrorVariable processError | ForEach-Object { 
		...
		If(($_.psIsContainer -eq $True)) {
			...
			If((IsVarDefined "matchstoprecurse") -and !($thisRelativePath -ieq "\") -and ($recurseSubDirs -eq $True)) {
				If(( ($thisRelativePath + "\" + $_.Name) -match $matchstoprecurse)) {$recurseSubDirs = $False; $noRecurseReason = (" matchstoprecurse ==> $thisRelativePath\" + $_.Name) }
			}
			
			# Check subdir against recursion in junctions
			If(($nofollowjunctions) -and !($thisRelativePath -ieq "\") -and ($recurseSubDirs -eq $True)) {
				If(($_.Attributes -band 1024)) {$recurseSubDirs = $False; $noRecurseReason = (" nofollowjunctions ==> $thisRelativePath\" + $_.Name) }
			}
			
			# Check subdir against maxdepth in recursion
			If(($recurseSubDirs -eq $True)) {
				If(($depth + 1) -gt $BkMaxDepth) {$recurseSubDirs = $False; $noRecurseReason = (" maxdepthlimit ==> $thisRelativePath\" + $_.Name) }
			}
			...
			If(($recurseSubDirs -eq $True)) {
				...
				$noRecurseReason | Out-File $BkCatalogExclude -encoding ASCII -append ; $scanThisFile = $False 
			}
			
		} Else {

			# Test we're quitting
			If((Check-UserCancelRequest)) { return; }
		
			If(!($thisRelativePath -ieq "\")) {
			
				# Increment the number of processed files
				$gP.FilesProcessed++
				$gP.BytesProcessed += $_.Length

				# Check Wether or Not This File Falls into
				# the selection criteria
				$scanThisFile = $scanThisPath
				
				# Clean up files ?
				If(($scanThisFile -eq $True) -and ((Get-ChildItem "Variable:matchcleanupfiles" -ErrorAction SilentlyContinue))) { 
					If(($_.Name -match $matchcleanupfiles)) { 
						Remove-Item -literalpath (Join-Path $folderPath $_.Name) -Force -ErrorAction SilentlyContinue | Out-Null
						if (!($?)) { Trace (" WARNING Failed to remove : " + (Join-Path $folderPath $_.Name)) $BkLogFile; $gP.WarningsCount += 1 } Else { Trace (" File Removed : " + (Join-Path $thisRelativePath $_.Name)) $BkLogFile }
						$scanThisFile = $False ; $isCleanedUp = $True
					}
				}
				
				# Archive Attribute : is it set as we need it ?
				If(($scanThisFile -eq $True) -and (($BkType -ieq "incr") -or ($BkType -ieq "diff")) -and !($_.Attributes -band 32) ) { 
					$scanThisFile = $False 
				}
				
				# Match Include ?
				If(($scanThisFile -eq $True) -and ((Get-ChildItem "Variable:matchincludefiles" -ErrorAction SilentlyContinue))) { 
					If(!($_.Name -match $matchincludefiles)) { $scanThisFile = $False }
					}
				
				# Match Exclude ?
				If(($scanThisFile -eq $True) -and ((Get-ChildItem "Variable:matchexcludefiles" -ErrorAction SilentlyContinue))) { 
					If(($_.Name -match $matchexcludefiles)) {
						" matchexcludefile ==> $thisRelativePath\" + $_.Name | Out-File $BkCatalogExclude -encoding ASCII -append ; $scanThisFile = $False 
						}
					}
				
				# Check the file falls into MaxFileAge and/or MinFileAge
				If(($scanThisFile -eq $True) -and ((Get-ChildItem "Variable:MaxFileAge" -ErrorAction SilentlyContinue) -or (Get-ChildItem "Variable:MinFileAge" -ErrorAction SilentlyContinue))) {
					If(($MaxFileAge)) {
						If(( $(New-TimeSpan $_.LastWriteTime $selectStart).Days -gt $MaxFileAge )) { $scanThisFile = $False }
					}
					If(($MinFileAge)) {
						If(( $(New-TimeSpan $_.LastWriteTime $selectStart).Days -lt $MinFileAge )) { $scanThisFile = $False }
					}
				}

				# Check the file falls into MaxFileSize and/or MinFileSize
				If(($scanThisFile -eq $True) -and ((Get-ChildItem "Variable:MaxFileSize" -ErrorAction SilentlyContinue) -or (Get-ChildItem "Variable:MinFileSize" -ErrorAction SilentlyContinue))) {
					If(($MaxFileSize)) {
						If( $_.Length -gt $MaxFileSize ) { $scanThisFile = $False }
					}
					If(($MinFileSize)) {
						If( $_.Length -lt $MinFileSize ) { $scanThisFile = $False }
					}
				}
				
				If(($scanThisFile -eq $True)) { 
					$gP.FilesSelected++ ; 
					$gP.BytesSelected += $_.Length ; 
					"$thisRelativePath\" + $_.Name | Out-File $BkCatalogInclude -encoding UTF8 -append 
					# Save Catalog Stats
					( $_.Extension + "," + [string]$gP.FilesSelected + "," + [string]$_.Length ) | Out-File $BkCatalogStats -encoding ASCII -append 
				}
				
				# Output Progress
				If(($scanThisPath -eq $true)) {
					If(($scanThisFile -eq $true)) {
						...
						If(!($isCleanedUp)) {
							Write-Progress -Activity "Folder $thisRelativePath" -Status "File Skip   : $_" -CurrentOperation ("Selected {0,0:n0} out of {1,0:n0} files in {2,0:n0} folders. {3,0:n2} MBytes to backup" -f  $gP.FilesSelected, $gP.FilesProcessed, $gP.FoldersDone, ($gP.BytesSelected/1mb))
						} Else {
							Write-Progress -Activity "Folder $thisRelativePath" -Status "File Delete : $_" -CurrentOperation ("Selected {0,0:n0} out of {1,0:n0} files in {2,0:n0} folders. {3,0:n2} MBytes to backup" -f  $gP.FilesSelected, $gP.FilesProcessed, $gP.FoldersDone, ($gP.BytesSelected/1mb))
						}
					}
				} Else {
	...
	If((Get-ChildItem "Variable:processError" -ErrorAction SilentlyContinue)) {
		For ($x=0; $x -lt $processError.Length; $x++) { $thisRelativePath + " : " + $processError[0].Exception.Message | Out-File $BkSelectionExcpt -encoding ASCII -append }
		Remove-Variable processError -ErrorAction SilentlyContinue | Out-Null
	}
...
Function Test-Path-Writable {
	...
	If((Test-Path -Path $testPath -PathType Container)) {
		...
		if (( $testType -ieq "file" )) {
			New-Item $dummyItem -type File -force -value "This is only a test file. You can delete it safely." -ErrorAction SilentlyContinue | Out-Null
		} Else {
			New-Item $dummyItem -type Directory -force -ErrorAction SilentlyContinue | Out-Null
		}
		If (($?)) {
			Remove-Item $dummyItem -ErrorAction SilentlyContinue | Out-Null
			if (!($?)) { $False } Else { $True }
		} Else { 
			$False 
		}
...
If($args.length -ne 0) { 
	...
	Write-Host $headerText ; " Wrong number of parameters. Try " + $thisScript.Name + " --help"; Write-Host " " ; return 
}
# --------------------------------------------------------------------
...
If(!($Test -eq 0)){ Write-Host $headerText ; " Wrong number of parameters. Try " + $thisScript.Name + " --help"; Write-Host " " ; return }

# --------------------------------------------------------------------
# Test the presence of a script in same directory holding 
# default variable values
# --------------------------------------------------------------------
$importCmd = (Join-Path $thisScript.Directory $thisScript.Name.Replace(".ps1", "-vars.ps1"))
...
do {
	switch ($args[$i]) 	{ 
		...
		Default { Write-Host $headerText ; " Unknown argument " + $args[$i] + " Try " + $thisScript.Name + " --help"; Write-Host " " ; return }
	}
	$i++; $i++
}
...
New-Item $BkRootDir -type Directory -ErrorAction SilentlyContinue | Out-Null
If(!$?) { 
	...
	If($thisScript.WinVer -lt 6 ) {
		New-Item (Join-Path $BkRootDir "__README__PLEASE__README__.txt") -type File -value "This directory contains Junctions.`nDO NOT DELETE THIS DIRECTORY AND IT'S CONTENTS USING WINDOWS EXPLORER.`nUse Junction -d to delete junctions and then safely delete the directory." -ErrorAction SilentlyContinue | Out-Null
	} Else {
		New-Item (Join-Path $BkRootDir "__README__PLEASE__README__.txt") -type File -value "This directory contains Symbolic Links.`nDO NOT DELETE THIS DIRECTORY AND IT'S CONTENTS USING WINDOWS EXPLORER.`nUse RD command delete symbolic links and then safely delete the directory." -ErrorAction SilentlyContinue | Out-Null
	}
	If(!$?) {
		Write-Host $headerText
...
Trace " 7Zip Switches   :  $Bk7ZipSwitches " $BkLogFile
Trace " " $BkLogFile
Trace " ------------------------------------------------------------------------------" $BkLogFile
...
Set-Variable -name BkLockFile -value (Join-Path $Env:Temp ($thisScript.Name.Substring(0, ($thisScript.Name.LastIndexOf("."))) + ".lock")) -scope Script
If((Test-Path $BkLockFile -pathType Leaf)) {
	...
	If ((New-TimeSpan -End (Get-Date) -Start (Get-ChildItem $BkLockFile).LastWriteTime).Hours -gt 72) { 
		Remove-Item $BkLockFile; New-Item $BkLockFile -ItemType File -Force -Value "~" | Out-Null  
	} Else {
	...
	New-Item $BkLockFile -ItemType File -Force -Value "~" | Out-Null
}
...
# --------------------------------------------------------------------
$includedSources = 0
$BkSelectionContents | Where-Object {$_ -match "^includesource=*"} | ForEach-Object {
...
If(( $includedSources -eq 0 )) {
	...
	If ((Test-Path ($BkRootDir) -PathType Container)) { DeleteRootDir $BkRootDir }
	if ((IsVarDefined "BkLockFile")) {if ((Test-Path ($BkLockFile) -PathType Leaf)) { Remove-Item -literalpath $BkLockFile -ErrorAction SilentlyContinue | Out-Null }}
	Return
...
# Go ahead only if we have at least one file to backup
If(($gP.FilesSelected -ge 1) -and !(Check-UserCancelRequest -eq $True)) {
	...
	If((Get-Item $BkSelectionExcpt).Length -gt 0) {
		...
		Trace (" WARNING " + $_) $BkLogFile; $gP.WarningsCount += 1
		}
	}
	...
	$Bk7ZipArgs += "`> `"$BkCompressDetail`""											# This is the catalog input file
	$Bk7ZipBatch = ("""$Bk7ZipBin"" {0} " -f (($Bk7ZipArgs -Join ' ')))	   				# This is the command line executed by the batch
	$Bk7ZipRetc = 0
	
	
	$Bk7ZipCmd = Join-Path $BkRootDir "7zBackup.bat"
	if((Test-Path $Bk7ZipCmd)) { remove-item $Bk7ZipCmd | Out-Null }
	New-Item $Bk7ZipCmd -ItemType File -Force -Value " " -ErrorAction SilentlyContinue | Out-Null
	"`@" + $Bk7ZipBatch | Out-File $Bk7ZipCmd -encoding UTF8 -append
	
	# Change progress 
	Write-Progress -Activity "Archiving into $BkDestFile" -Status "Please wait ..." -CurrentOperation "7-Zip is working (you can check your archive growing in a separate window)"
	
	# Start the clocks
	$compressStart = Get-Date

	# 7Zip-it-up and append to the log of operations
	#if(!(Check-UserCancelRequest -eq $True)) { &$Bk7ZipCmd ; $Bk7ZipRetc = $LASTEXITCODE }
	If(!(Check-UserCancelRequest -eq $True)) { CMD /C """"$Bk7ZipBatch"""" ; $Bk7ZipRetc = $LASTEXITCODE }
	if((Test-Path $Bk7ZipCmd)) { remove-item $Bk7ZipCmd | Out-Null }
...
# Clean Up 
Set-Location ($thisScript.StartDir)
CleanUp
