...
Public Class utlWin32_Perm
    ...
    Private Shared Function GetFileSecurity( _
    ...
    <DllImport("AdvAPI32.DLL", CharSet:=CharSet.Auto, SetLastError:=True)> _
    Private Shared Function GetSecurityDescriptorDacl( _
    ...
    <DllImport("AdvAPI32.DLL", CharSet:=CharSet.Auto, SetLastError:=True)> _
    Private Shared Function GetAce( _
    ...
    Public Shared Function FindFilesAndDirs(ByVal dirName As String, Optional ByVal types As FindType = FindType.Directories, Optional ByVal recursive As Boolean = False, Optional ByVal iMax As Integer = 100000, Optional ByVal iLevel As Integer = 0) As List(Of String)
        ...
        If findHandle <> INVALID_HANDLE_VALUE Then 'valid handle
            ...
            Do 'Loop through files/folders
                ...
                If (CInt(findData.dwFileAttributes) And FILE_ATTRIBUTE_DIRECTORY) <> 0 Then 'if current object is a directory
                    ...
                    End If
                Else
                    If (types And FindType.Files) = FindType.Files Then 'If we specified files, get their info
                        results.Add(System.IO.Path.Combine(dirName, currentFileName))
                    End If
                End If
...
"@
Add-Type -ReferencedAssemblies ("System.Windows.Forms") -Language VisualBasic -IgnoreWarnings -TypeDefinition $ass
function Get-mypath{
$mypath = $psISE.CurrentFile.Fullpath
if ($mypath -eq $null) {$mypath = $MyInvocation.ScriptName}
return $mypath 
}
...
if ($aPath[0] -eq '') {
    ...
    $server_long = $apath[2]
    $null = $server_long -match "\w*" #get rid of full name
    $server = $matches[0]

    $outfile = $path -replace "\\",""
    $outfile = $outfile -replace $server_long,"$server`_"

    } else {$server = $env:COMPUTERNAME;$outfile = "$server`_$path"}

$outfile = "perm_" + $outfile.replace("\","_")
$outfile = $outfile.replace(":","")

$mypath = get-mypath
$cwd = $mypath.substring(0,$mypath.LastIndexOf("\")) #(Get-Location).Path


if (($progress -eq $true) -and ($LevelsDeep -ne 0)) {
        $folders =[utlwin32_dir]::FindFilesAndDirs($path,2,$true,$LevelsDeep)
        $i=0
        $results = @()
        $folders | %{
            $i+=1
             Write-Progress -Activity "Scanning permission $_" -status "Checking folder $i of $($folders.count)" -percentComplete ($i / $folders.count*100)
            $results +=  [utlwin32_perm]::GetObjectACL($_,$true)
        }
        
    } else  {"Scanning $path...";$results = [utlwin32_perm]::GetFolderPermR($path,$LevelsDeep)}

    

    if ($IncludeServer -eq $true) {$results | Add-Member –MemberType NoteProperty –Name ComputerName –Value $server}
    if ($IncludeFileName -eq $true) {$results | Add-Member –MemberType NoteProperty –Name FileName –Value "$outfile.csv"}
    

switch ($output) 
    { 
        "file" {"Results saved as $cwd\$outfile.csv";$results | export-csv -NoTypeInformation "$cwd\$outfile.csv"} 
        "notepad" {$results | export-csv -NoTypeInformation "$cwd\$outfile.csv";notepad $cwd\$outfile.csv} 
